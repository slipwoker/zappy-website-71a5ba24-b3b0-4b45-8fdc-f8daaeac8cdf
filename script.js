document.addEventListener('DOMContentLoaded', function () {
  ...
  function validateField(field) {
    ...
    if (field.tagName === 'TEXTAREA' && field.dataset.minlength) {
      const min = parseInt(field.dataset.minlength, 10);
      if (!isNaN(min) && value.length < min) {
        setFieldError(field, field.dataset.errorMinlength || 'Message is too short.');
        return false;
      }
    }
    ...
    fetch(action, {
      ...
      .then(function (response) {
        if (response.ok) {
          ...
          showFormFeedback(form, errorMessage, 'error');
        }
      })
      .catch(function () {
        showFormFeedback(form, errorMessage, 'error');
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "מהרעיון לסירטון משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
    ...
    (function() {
        ...
        function initContactForm() {
            ...
            try {
                ...
                const payload = {
                    websiteId: '71a5ba24-b3b0-4b45-8fdc-f8daaeac8cdf',
                    name: name,
                    email: email,
                    subject: subject,
                    message: message,
                ...
                if (result.success) {
                    ...
                    if (result.thankYouPagePath && result.ticketNumber) {
                        ...
                        if (isPreviewMode && window.ZAPPY_CONFIG) {
                            ...
                            const websiteId = window.ZAPPY_CONFIG.websiteId || '71a5ba24-b3b0-4b45-8fdc-f8daaeac8cdf';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '71a5ba24-b3b0-4b45-8fdc-f8daaeac8cdf';
